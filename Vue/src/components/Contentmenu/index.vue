...
import { createFile_API, createFileState_API } from "@/api/file";
...
async function dialogConfirm() {
  ...
  if (createDialog.type === "folder") {
    ...
    // 创建文件状态
    try {
      const stateRes = await createFileState_API({
        fileid: res.data,
        userid
      });

      if (stateRes.code !== 200) {
        console.warn("文件状态记录创建失败:", stateRes.msg);
      }
    } catch (error) {
      console.error("创建文件状态记录时出错:", error);
    }
